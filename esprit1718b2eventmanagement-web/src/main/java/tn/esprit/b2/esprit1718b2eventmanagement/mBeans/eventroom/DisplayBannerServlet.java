...
/**
 * Servlet implementation class DisplayBannerServelet
 */
@WebServlet(urlPatterns = { "/image" })
public class DisplayBannerServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		SpecialEvent event = specialEventServicesLocal.find(id);
		 if (event == null) {
		 ...
		 response.setHeader("Content-Length", String.valueOf(getBytesFromFile(event.getEventBanner()).length));
		 response.setHeader("Content-Disposition", "inline; filename=\"" + event.getId() + "\"");
		
		 // Write image data to Response.
		 response.getOutputStream().write(getBytesFromFile(event.getEventBanner()));
